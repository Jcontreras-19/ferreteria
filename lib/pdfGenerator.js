...
export function generateQuotesSummaryPDF(quotes, periodType, startDate, endDate) {
  ...
  doc.setFillColor(GRC_COLORS.dark[0], GRC_COLORS.dark[1], GRC_COLORS.dark[2])
  doc.rect(0, 8, pageWidth, 65, 'F')

  // Logo GRC a la izquierda
  drawGRCLogo(doc, margin, 15, 30, 30)

  // Información de la empresa en el centro-izquierda (después del logo, bien separada)
  const logoEndX = margin + 32
  const companyStartX = logoEndX + 10
  const companyMaxWidth = 90 // Ancho máximo para evitar solapamiento
  
  doc.setTextColor(255, 255, 255)
  
  // Nombre de la empresa - tamaño moderado
  doc.setFontSize(16)
  doc.setFont('helvetica', 'bold')
  doc.text('CORPORACIÓN GRC', companyStartX, 22, { maxWidth: companyMaxWidth })
  
  // Subtítulo de la empresa
  doc.setFontSize(9)
  doc.setFont('helvetica', 'normal')
  doc.text('SERVICIOS GENERALES', companyStartX, 28, { maxWidth: companyMaxWidth })
  
  // Información adicional de la empresa
  doc.setFontSize(7)
  doc.text('SERVICIOS DE APOYO A LAS EMPRESAS', companyStartX, 33, { maxWidth: companyMaxWidth })
  doc.text('ISO 9001:2015', companyStartX, 38, { maxWidth: companyMaxWidth })
  
  // Dirección
  doc.setFontSize(6)
  doc.text('Av. José Gálvez 1322 Dpto. 302', companyStartX, 43, { maxWidth: companyMaxWidth })
  doc.text('La Perla - Callao', companyStartX, 47, { maxWidth: companyMaxWidth })

  // Título del reporte centrado y destacado (bien separado del nombre de la empresa)
  const titleStartX = pageWidth / 2
  doc.setFontSize(18)
  doc.setFont('helvetica', 'bold')
  doc.text('REPORTE DE COTIZACIONES', titleStartX, 35, { align: 'center' })

  // Información del reporte a la derecha (bien separada)
  ...
  doc.text(`Período: ${periodLabels[periodType] || periodType}`, rightMargin, 22, { align: 'right' })
  
  const dateRange = `${startDate.toLocaleDateString('es-PE')} - ${endDate.toLocaleDateString('es-PE')}`
  doc.text(`Rango: ${dateRange}`, rightMargin, 28, { align: 'right' })
  ...
  // Diseño compacto: 4 tarjetas en 2 filas (2x2)
  const cardWidth = (pageWidth - (margin * 2) - 15) / 2 // 15px de espacio entre tarjetas
  const cardHeight = 20 // Más compacto
  const cardSpacing = 15
  const rowSpacing = 10

  // Fila 1: Total Cotizaciones y Monto Total
  // Tarjeta 1: Total de Cotizaciones (izquierda)
  doc.setFillColor(GRC_COLORS.primary[0], GRC_COLORS.primary[1], GRC_COLORS.primary[2])
  doc.setDrawColor(GRC_COLORS.dark[0], GRC_COLORS.dark[1], GRC_COLORS.dark[2])
  doc.setLineWidth(0.5)
  doc.roundedRect(margin, yPos - 6, cardWidth, cardHeight, 2, 2, 'FD')
  doc.setTextColor(255, 255, 255)
  doc.setFontSize(8)
  doc.setFont('helvetica', 'normal')
  doc.text('Total Cotizaciones', margin + 3, yPos - 1)
  doc.setFontSize(16)
  doc.setFont('helvetica', 'bold')
  doc.text(totalQuotes.toString(), margin + 3, yPos + 8)

  // Tarjeta 2: Monto Total (derecha)
  doc.setFillColor(GRC_COLORS.dark[0], GRC_COLORS.dark[1], GRC_COLORS.dark[2])
  doc.roundedRect(margin + cardWidth + cardSpacing, yPos - 6, cardWidth, cardHeight, 2, 2, 'FD')
  doc.setTextColor(255, 255, 255)
  doc.setFontSize(8)
  doc.setFont('helvetica', 'normal')
  doc.text('Monto Total', margin + cardWidth + cardSpacing + 3, yPos - 1)
  doc.setFontSize(14)
  doc.setFont('helvetica', 'bold')
  const totalAmountText = `S/. ${totalAmount.toFixed(2)}`
  doc.text(totalAmountText, margin + cardWidth + cardSpacing + 3, yPos + 8)

  yPos += cardHeight + rowSpacing

  // Fila 2: Aprobadas y Pendientes
  // Tarjeta 3: Aprobadas (izquierda)
  doc.setFillColor(34, 197, 94) // green-500
  doc.roundedRect(margin, yPos - 6, cardWidth, cardHeight, 2, 2, 'FD')
  doc.setTextColor(255, 255, 255)
  doc.setFontSize(8)
  doc.setFont('helvetica', 'normal')
  doc.text('Aprobadas', margin + 3, yPos - 1)
  doc.setFontSize(16)
  doc.setFont('helvetica', 'bold')
  doc.text(approvedCount.toString(), margin + 3, yPos + 8)

  // Tarjeta 4: Pendientes (derecha)
  doc.setFillColor(251, 191, 36) // yellow-400
  doc.roundedRect(margin + cardWidth + cardSpacing, yPos - 6, cardWidth, cardHeight, 2, 2, 'FD')
  doc.setTextColor(255, 255, 255)
  doc.setFontSize(8)
  doc.setFont('helvetica', 'normal')
  doc.text('Pendientes', margin + cardWidth + cardSpacing + 3, yPos - 1)
  doc.setFontSize(16)
  doc.setFont('helvetica', 'bold')
  doc.text(pendingCount.toString(), margin + cardWidth + cardSpacing + 3, yPos + 8)
