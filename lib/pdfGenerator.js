...
export function generateQuotesSummaryPDF(quotes, periodType, startDate, endDate) {
  ...
  doc.setFontSize(14)
  doc.setFont('helvetica', 'normal')
  doc.text('REPORTE DE COTIZACIONES', pageWidth - margin, 28, { align: 'right' })
  ...
  doc.text(`Período: ${periodLabels[periodType] || periodType}`, pageWidth - margin, 35, { align: 'right' })
  
  const dateRange = `${startDate.toLocaleDateString('es-PE')} - ${endDate.toLocaleDateString('es-PE')}`
  doc.text(dateRange, pageWidth - margin, 42, { align: 'right' })
  ...
  doc.setFont('helvetica', 'bold')
  doc.text(`Total de Cotizaciones: ${totalQuotes}`, margin, yPos)
  yPos += 7

  doc.text(`Monto Total: S/. ${totalAmount.toFixed(2)}`, margin, yPos)
  yPos += 7
  ...
  Object.entries(statusCounts).forEach(([status, count]) => {
    doc.text(`  • ${statusLabels[status] || status}: ${count}`, margin + 5, yPos)
    yPos += 6
  })
  ...
  doc.text('Estado', margin + 70, yPos)
  doc.text('Monto', margin + 95, yPos)
  doc.text('Fecha', margin + 120, yPos)
  yPos += 10
  ...
  quotes.forEach((quote, index) => {
    ...
    const quoteNumber = quote.quoteNumber ? `#${quote.quoteNumber}` : '-'
    const clientName = quote.name.length > 25 ? quote.name.substring(0, 22) + '...' : quote.name
    const status = statusLabels[quote.status] || quote.status
    ...
    doc.text(quoteNumber, margin + 2, yPos)
    doc.text(clientName, margin + 15, yPos)
    doc.text(status, margin + 70, yPos)
    doc.text(amount, margin + 95, yPos)
    doc.text(date, margin + 120, yPos)
