...
export function generateQuotesSummaryPDF(quotes, periodType, startDate, endDate) {
  ...
  doc.setFontSize(22)
  doc.setFont('helvetica', 'bold')
  doc.text('CORPORACIÓN GRC', infoStartX, 22)
  
  doc.setFontSize(12)
  doc.setFont('helvetica', 'normal')
  doc.text('SERVICIOS GENERALES', infoStartX, 28)
  
  doc.setFontSize(9)
  doc.text('SERVICIOS DE APOYO A LAS EMPRESAS', infoStartX, 33)
  doc.text('ISO 9001:2015', infoStartX, 38)
  
  doc.setFontSize(7)
  doc.text('Av. José Gálvez 1322 Dpto. 302 La Perla - Callao', infoStartX, 43)

  // Información del reporte a la derecha
  doc.setFontSize(11)
  doc.setFont('helvetica', 'bold')
  doc.text('REPORTE DE COTIZACIONES', pageWidth - margin, 22, { align: 'right' })
  ...
  // Agrupar estados según la lógica unificada
  const statusCounts = {
  ...
  // Tarjeta 1: Total de Cotizaciones
  doc.setFillColor(GRC_COLORS.primary[0], GRC_COLORS.primary[1], GRC_COLORS.primary[2])
  doc.setDrawColor(GRC_COLORS.dark[0], GRC_COLORS.dark[1], GRC_COLORS.dark[2])
  doc.setLineWidth(0.5)
  doc.roundedRect(margin, yPos - 8, 85, 20, 3, 3, 'FD')
  doc.setTextColor(255, 255, 255)
  doc.setFontSize(9)
  doc.setFont('helvetica', 'normal')
  doc.text('Total Cotizaciones', margin + 3, yPos - 2)
  doc.setFontSize(16)
  doc.setFont('helvetica', 'bold')
  doc.text(totalQuotes.toString(), margin + 3, yPos + 6)

  // Tarjeta 2: Monto Total
  doc.setFillColor(GRC_COLORS.dark[0], GRC_COLORS.dark[1], GRC_COLORS.dark[2])
  doc.roundedRect(margin + 90, yPos - 8, 85, 20, 3, 3, 'FD')
  doc.setTextColor(255, 255, 255)
  doc.setFontSize(9)
  doc.setFont('helvetica', 'normal')
  doc.text('Monto Total', margin + 93, yPos - 2)
  doc.setFontSize(14)
  doc.setFont('helvetica', 'bold')
  doc.text(`S/. ${totalAmount.toFixed(2)}`, margin + 93, yPos + 6)
  ...
  doc.setFontSize(9)
  doc.setFont('helvetica', 'normal')
  doc.text('Promedio', margin + 3, yPos - 2)
  doc.setFontSize(14)
  doc.setFont('helvetica', 'bold')
  doc.text(`S/. ${avgAmount.toFixed(2)}`, margin + 3, yPos + 6)

  yPos += 18

  // Distribución por Estado con diseño mejorado
  doc.setTextColor(GRC_COLORS.darker[0], GRC_COLORS.darker[1], GRC_COLORS.darker[2])
  doc.setFontSize(12)
  doc.setFont('helvetica', 'bold')
  ...
  Object.entries(statusCounts).forEach(([status, count], index) => {
    ...
    const color = statusColors[status] || [200, 200, 200]
    ...
    doc.setFontSize(8)
    doc.setFont('helvetica', 'bold')
    doc.text(statusLabels[status] || status, boxX + 2, boxY + 6)
    doc.setFontSize(10)
    doc.text(count.toString(), boxX + 2, boxY + 12)
  ...
  // Filas de cotizaciones con diseño mejorado
  doc.setTextColor(GRC_COLORS.text[0], GRC_COLORS.text[1], GRC_COLORS.text[2])
  doc.setFont('helvetica', 'normal')
  doc.setFontSize(8)
