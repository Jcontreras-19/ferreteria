...
export function generateQuotesSummaryPDF(quotes, periodType, startDate, endDate) {
  ...
  // Título del reporte en una línea propia, centrado y bien separado (más abajo)
  doc.setFontSize(20)
  doc.setFont('helvetica', 'bold')
  doc.text('REPORTE DE COTIZACIONES', pageWidth / 2, 62, { align: 'center' })
  ...
  // Diseño compacto: 4 tarjetas en 1 fila horizontal
  const totalSpacing = 10 // Espacio entre tarjetas
  ...
  const cardHeight = 24 // Altura compacta pero legible
  ...
  cardLabels.forEach((label, index) => {
    ...
    doc.setLineWidth(0.3)
    doc.roundedRect(cardX, yPos - 6, cardWidth, cardHeight, 2, 2, 'FD')
    
    // Texto del label
    doc.setTextColor(255, 255, 255)
    doc.setFontSize(7)
    doc.setFont('helvetica', 'normal')
    doc.text(label, cardX + 3, yPos - 1, { maxWidth: cardWidth - 6 })
    
    // Valor destacado
    doc.setFontSize(14)
    doc.setFont('helvetica', 'bold')
    const valueText = cardValues[index]
    doc.text(valueText, cardX + 3, yPos + 11, { maxWidth: cardWidth - 6 })
  })
