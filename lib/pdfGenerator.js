...
export function generateQuotePDF(quote) {
  ...
  // SUBTOTAL - Centrado
  doc.text('SUBTOTAL:', summaryStartX + summaryWidth / 2, summaryY, { align: 'center' })
  doc.setFont('helvetica', 'bold')
  doc.text(`S/. ${subtotal.toFixed(2)}`, summaryStartX + summaryWidth / 2, summaryY + 5, { align: 'center' })
  
  doc.setFont('helvetica', 'normal')
  doc.text('DESCUENTO:', summaryStartX + summaryWidth / 2, summaryY + 12, { align: 'center' })
  doc.setFont('helvetica', 'bold')
  doc.text(`S/. ${discount.toFixed(2)}`, summaryStartX + summaryWidth / 2, summaryY + 17, { align: 'center' })

  // Línea separadora antes del total
  doc.setDrawColor(GRC_COLORS.dark[0], GRC_COLORS.dark[1], GRC_COLORS.dark[2])
  doc.setLineWidth(0.5)
  doc.line(summaryStartX + 5, summaryY + 22, summaryStartX + summaryWidth - 5, summaryY + 22)

  // Total destacado con fondo verde - CENTRADO
  const totalY = summaryY + 24
  const totalHeight = 10
  const totalStartX = summaryStartX + 2
  const totalWidth = summaryWidth - 4
  
  // Fondo verde para el TOTAL
  doc.setFillColor(GRC_COLORS.primary[0], GRC_COLORS.primary[1], GRC_COLORS.primary[2])
  doc.rect(totalStartX, totalY - 2, totalWidth, totalHeight, 'F')
  
  // Borde verde más oscuro
  doc.setDrawColor(GRC_COLORS.dark[0], GRC_COLORS.dark[1], GRC_COLORS.dark[2])
  doc.setLineWidth(0.5)
  doc.rect(totalStartX, totalY - 2, totalWidth, totalHeight, 'S')
  
  // Texto del total en blanco - CENTRADO
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(14)
  doc.setTextColor(255, 255, 255)
  const totalAmount = (subtotal - discount).toFixed(2)
  const totalCenterY = totalY - 2 + totalHeight / 2 + 3
  doc.text('TOTAL:', totalStartX + totalWidth / 2, totalCenterY - 2, { align: 'center' })
  doc.setFontSize(16)
  doc.text(`S/. ${totalAmount}`, totalStartX + totalWidth / 2, totalCenterY + 3, { align: 'center' })
