...
export function generateQuotePDF(quote) {
  ...
  products.forEach((product) => {
    ...
    const calculatedRowHeight = Math.max((nameLines.length * 4) + (descLines.length * 4) + 2, 10)
    
    // Ajustar altura de la fila
    const actualRowHeight = calculatedRowHeight
    const textStartY = yPos - actualRowHeight + 6
    ...
    if (productDesc && descLines.length > 0) {
      ...
      const descY = textStartY + (nameLines.length * 4) + 1
    ...
    const centerY = yPos - actualRowHeight / 2 + 2
